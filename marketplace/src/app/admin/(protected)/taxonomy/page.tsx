    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/60">
            Управляйте брендами, моделями, цветами и другими общими атрибутами для карточек и фильтров.
          </p>
        </div>
        <Link
          href="/admin/vehicles"
          ...
          return (
            <Link
              ...
              className={`rounded-full px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] transition ${
                active ? "bg-white/10 text-white border border-white/20" : "text-white/60 hover:text-white hover:bg-white/5"
              }`}
            >
              {item.label}
            </Link>
      ...
      <div className="grid gap-4 md:grid-cols-[2fr,1fr]">
                ...
                {entries.length === 0 ? (
                  ...
                  entries.map((item) => (
                    <tr key={item.id} className="border-b border-white/5 last:border-none">
                      ...
                      <td className="px-4 py-3 text-white/80">{item.value}</td>
                      <td className="px-4 py-3 text-white/60">{item.parentValue || "—"}</td>
                      <td className="px-4 py-3 text-right">
                        <form action={deleteTaxonomyAction}>
                          <input type="hidden" name="id" value={item.id} />
                          <input type="hidden" name="type" value={selectedType} />
                          <button className="text-xs text-red-300 hover:text-red-200" type="submit">
                            Удалить
                          </button>
                        </form>
                      </td>
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/6 p-4 text-sm text-white/80">
          <h3 className="text-base font-semibold text-white">Добавить / обновить</h3>
          <p className="text-xs text-white/55">Используйте одинаковые значения в поле «Значение» для консистентности фильтров.</p>
          <form action={upsertTaxonomyAction} className="mt-4 space-y-3">
            <input type="hidden" name="type" value={selectedType} />
            <label className="space-y-1 block">
              <span className="text-xs uppercase tracking-[0.14em] text-white/50">Метка</span>
              <input
                name="label"
                required
            ...
            <label className="space-y-1 block">
              <span className="text-xs uppercase tracking-[0.14em] text-white/50">Значение (латиница)</span>
              <input
                name="value"
                required
                className="h-10 w-full rounded-lg border border-white/15 bg-black/30 px-3 text-sm text-white placeholder:text-white/35 focus:border-brand-primary focus:outline-none"
            ...
            <label className="space-y-1 block">
              <span className="text-xs uppercase tracking-[0.14em] text-white/50">Родитель (для моделей укажите бренд)</span>
              <input
                name="parentValue"
                className="h-10 w-full rounded-lg border border-white/15 bg-black/30 px-3 text-sm text-white placeholder:text-white/35 focus:border-brand-primary focus:outline-none"
            ...
            <label className="space-y-1 block">
              <span className="text-xs uppercase tracking-[0.14em] text-white/50">Сортировка</span>
              <input
                name="sortOrder"
                type="number"
                defaultValue={0}
